...
class ThreadSafeInputList:
    ...
    def get_last(self, is_clear=False):
        ...
        if len(self.inputs) == 0:
            ...
            value = self.inputs[-1]
            # if is_clear is set, clear out the input list after reading the last input
            if is_clear:
                self.inputs = list()
...
class UserInputModule:
    ...
    def getInput(self):
        # if using asynchronous input
        if self.is_asyn:
            return self.inputs.get_last(is_clear=True)
        # if not using asynchronous input (wait for the user's input)
        else:
